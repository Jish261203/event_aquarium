...
const Card = async ({ event, hasOrderLink, hidePrice }: CardProps) => {
  ...
  // Ensure event and event.organizer exist before accessing properties
  const isEventCreator =
    mongoUserId && event?.organizer?._id
      ? mongoUserId === event.organizer._id.toString()
      : false;
  ...
  return (
    <div className="group relative flex min-h-[380px] w-full max-w-[400px] flex-col overflow-hidden rounded-xl bg-white shadow-md transition-all hover:shadow-lg md:min-h-[438px]">
      <Link
        ...
        style={{
          backgroundImage: `url(${event?.imageUrl || "/default-image.jpg"})`,
        }}
        className="flex-center flex-grow bg-gray-50 bg-cover bg-center text-grey-500"
      />

      {isEventCreator && !hidePrice && event?.organizer && (
